    ...
    $scope.loginEmail = function(formName,cred) {
      ...
      if(formName.$valid) {  // Check if the form data is valid or not
          ...
          firebase.auth().signInWithEmailAndPassword(cred.email,cred.password).then(function(result) {
            ...
            function(error) {
              sharedUtils.hideLoading();
              sharedUtils.showAlert("警告!!","授權錯誤");
            }
        );

      }else{
        sharedUtils.showAlert("警告!!","你輸入的格式不正確");
      }
...
.controller('settingsCtrl', function($scope,$rootScope,fireBaseData,$firebaseObject,
    ...
    $scope.addManipulation = function(edit_val) {  // Takes care of address add and edit ie Address Manipulator
      ...
      if(edit_val!=null) {
        ...
        var title="編輯地址";
        var sub_title="編輯你的地址";
      }
      else {
        $scope.data = {};    // For adding new address
        var title="新增地址";
        var sub_title="新增你的地址";
      }
      // An elaborate, custom popup
      var addressPopup = $ionicPopup.show({
        template: '<input type="text"   placeholder="姓名"  ng-model="data.nickname"> <br/> ' +
                  '<input type="text"   placeholder="地址" ng-model="data.address"> <br/> ' +
                  '<input type="number" placeholder="郵遞區號" ng-model="data.pin"> <br/> ' +
                  '<input type="number" placeholder="聯絡電話" ng-model="data.phone">',
        title: title,
        subTitle: sub_title,
        scope: $scope,
        buttons: [
    ...
    $scope.deleteAddress = function(del_id) {
      var confirmPopup = $ionicPopup.confirm({
        title: '刪除地址',
        template: '你確定你要刪除這個地址嗎?',
        buttons: [
          { text: '否' , type: 'button-stable' },
          { text: '是', type: 'button-assertive' , onTap: function(){return del_id;} }
        ]
      });
    ...
    $scope.save= function (extras,editable) {
      ...
      if(editable.email!="" && editable.email!=null  && editable.email!=$scope.user_info.email){
        ...
        firebase.auth().currentUser.updateEmail(editable.email).then(function(ok) {
          ...
          sharedUtils.showAlert("錯誤!!",error);
        });
      }
...
.controller('checkoutCtrl', function($scope,$rootScope,sharedUtils,$state,$firebaseArray,
    ...
    $scope.pay=function(address,payment){
      ...
      if(address==null || payment==null){
        ...
        sharedUtils.showAlert("錯誤!!","請選擇地址及付款方式")
      }
      else {
        ...
        sharedUtils.showAlert("提示!!", "點餐成功");

        // Go to past order page
        $ionicHistory.nextViewOptions({
          historyRoot: true
        });
        $state.go('lastOrders', {}, {location: "取代", reload: true});
      }
    }
